...
if __name__ == '__main__':
    ...
    with open('dataset/dataSet.json') as dataset_file, open("stateEstimation.csv", "w") as csv:
        ...
        for i in xrange(0, total_imgs + 1):
            ...
            print >> csv, "%s,%s,%d,%d,%.1f" % (ref, "Board X", exp_bx, real_bx, error(exp_bx, real_bx))
            print >> csv, "%s,%s,%d,%d,%.1f" % (ref, "Board Y", exp_by, real_by, error(exp_by, real_by))
            print >> csv, "%s,%s,%d,%d,%.1f" % (ref, "Board Width", exp_bw, real_bw, error(exp_bw, real_bw))
            print >> csv, "%s,%s,%d,%d,%.1f" % (ref, "Board Height", exp_bh, real_bh, error(exp_bh, real_bh))
            ...
            if 'hole' in real_board:
                ...
                if exp_chr > 0:
                    print >> csv, "%s,%s,%d,%d,%.1f" % (ref, "Cornhole X", exp_chx, real_chx, error(exp_chx, real_chx))
                    print >> csv, "%s,%s,%d,%d,%.1f" % (ref, "Cornhole Y", exp_chy, real_chy, error(exp_chy, real_chy))
                    print >> csv, "%s,%s,%d,%d,%.1f" % (ref, "Cornhole Radius", exp_chr, real_chr, error(exp_chr, real_chr))
